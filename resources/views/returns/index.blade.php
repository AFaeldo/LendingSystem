...
<div class="flex flex-col sm:flex-row items-start sm:items-center justify-end gap-4 mb-6">
    <a href="{{ route('returns.create') }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2.5 rounded-xl font-semibold text-sm transition shadow-sm shrink-0">
        ...
        @forelse($returns as $return)
            <div class="p-4 space-y-3.5">
                <div class="flex items-start justify-between gap-3">
                    ...
                    <span class="px-2.5 py-0.5 text-xs font-medium rounded-full border shrink-0 bg-green-50 border-green-100 text-green-700">
                        Returned
                    </span>
                </div>
                ...
                <div class="grid grid-cols-2 gap-2 bg-slate-50/60 rounded-xl p-3 border border-slate-100/80 text-xs">
                    ...
                    <div>
                        ...
                        <span class="font-medium text-slate-700">{{ optional($return->returned_at)->format('M d, Y') }}</span>
                    </div>
                </div>

                <div class="pt-1">
                    <a href="{{ route('returns.show', $return) }}" class="w-full inline-flex items-center justify-center gap-1 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 py-2 rounded-xl text-xs font-semibold transition">
