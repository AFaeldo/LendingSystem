...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-4 mb-6">

        <div class="flex items-center gap-2 w-full sm:w-auto">
            {{-- VIEW ARCHIVE LINK BUTTON --}}
            <a href="{{ route('borrowers.archive.index') }}"
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div class="hidden md:block overflow-x-auto">
            <table class="w-full text-sm">
                <thead>
                    <tr class="bg-slate-50/70 border-b border-slate-100 text-left text-gray-500 text-xs font-semibold uppercase tracking-wider">
                        ...
                        <tr class="hover:bg-gray-50/50 transition">
                            ...
                            <td class="px-6 py-4 text-right whitespace-nowrap">
                                <div class="inline-flex items-center gap-2">
                                    {{-- EDIT CONTROL ICON --}}
                                    <a href="{{ route('borrowers.edit', $b) }}"
                                        class="inline-flex items-center justify-center gap-1 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 h-8 px-3 rounded-lg text-xs font-semibold transition shadow-sm" title="Edit Profile">
                                        <i class="ti ti-edit text-sm text-slate-400"></i> Edit
                                    </a>

                                    {{-- ARCHIVE ACTION CONTROL ICON --}}
                                    <form action="{{ route('borrowers.archive', $b) }}" method="POST" class="inline-block"
                                        onsubmit="return confirm('Archive borrower profile? This will temporarily remove them from the active registries.')">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                            class="inline-flex items-center justify-center gap-1 bg-amber-50 hover:bg-amber-100 border border-amber-100 text-amber-700 h-8 px-3 rounded-lg text-xs font-semibold transition shadow-sm cursor-pointer" title="Archive Profile">
                                            <i class="ti ti-archive text-sm text-amber-500/80"></i> Archive
                                        </button>
                                    </form>
                                </div>
                            </td>
                        ...
                        <tr>
                            <td colspan="9" class="px-6 py-16 text-center text-gray-400">
                                <div class="flex flex-col items-center justify-center space-y-2">
                                    ...
                                    <span class="text-sm">No active borrower records found.</span>
                                </div>
                            </td>
                        </tr>
                    @endforelse
        ...
        {{-- MOBILE CONTROLS CARD COMPONENT --}}
            ...
            @forelse($borrowers as $b)
                <div class="p-4 space-y-3.5">
                    ...
                    <div class="grid grid-cols-2 gap-2 pt-1">
                        <a href="{{ route('borrowers.edit', $b) }}"
                            class="inline-flex items-center justify-center bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-700 py-2 rounded-xl text-xs font-semibold transition gap-1.5">
                            <i class="ti ti-edit text-sm text-slate-400"></i> Edit Profile
                        </a>

                        <form action="{{ route('borrowers.archive', $b) }}" method="POST" class="w-full"
                            onsubmit="return confirm('Archive borrower profile?')">
                            @csrf
                            @method('PATCH')
                            <button type="submit"
                                class="w-full inline-flex items-center justify-center bg-amber-50 hover:bg-amber-100 border border-amber-100 text-amber-700 py-2 rounded-xl text-xs font-semibold transition cursor-pointer gap-1.5">
                                <i class="ti ti-archive text-sm text-amber-500/80"></i> Archive
                            </button>
                        </form>
                    </div>
